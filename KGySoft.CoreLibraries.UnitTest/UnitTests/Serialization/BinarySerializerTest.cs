        ...
        private class TestWriter : BinaryWriter
        {
            ...
            public override void Write(byte value)
            {
                ...
                if (log)
                {
                    ...
                    string valueStr = name == "WriteDataType"
                        ? Reflector.InvokeMethod(typeof(BinarySerializationFormatter), "DataTypeToString", (ushort)value).ToString()
                        : value.ToString("X2");
            ...
            public override void Write(ushort value)
            {
                ...
                if (log)
                {
                    ...
                    string name = frames[1].GetMethod().Name;
                    string valueStr = name == "WriteDataType"
                        ? Reflector.InvokeMethod(typeof(BinarySerializationFormatter), "DataTypeToString", value).ToString()
                        : value.ToString("X4");
        ...
        private class TestReader : BinaryReader
        {
            ...
            public override ushort ReadUInt16()
            {
                ...
                if (log)
                {
                    var frames = new StackTrace().GetFrames();
                    string name = frames[1].GetMethod().Name;
                    string valueStr = name == "ReadDataType"
                        ? Reflector.InvokeMethod(typeof(BinarySerializationFormatter), "ToString", result).ToString()
                        : result.ToString("X4");
                    Console.WriteLine($"ushort: {result} ({valueStr}) - {name}");
                }
                return result;
            }
