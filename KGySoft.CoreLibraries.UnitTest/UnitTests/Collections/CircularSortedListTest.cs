...
namespace KGySoft.CoreLibraries.UnitTests.Collections
{
    ...
    public class CircularSortedListTest
    {
        ...
        public void Populate()
        {
            ...
            // first element
            Assert.IsTrue(cslist.TryAdd(1, 1));
            Assert.IsFalse(cslist.TryAdd(1, 1));
            Assert.AreEqual(0, cslist.IndexOfKey(1));

            // 1 element, add to last
            Assert.AreEqual(1, cslist.Add(6, 6));
            Assert.IsTrue(cslist.Keys.SequenceEqual([1, 6]));
