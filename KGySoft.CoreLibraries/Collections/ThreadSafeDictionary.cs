...
namespace KGySoft.Collections
{
    ...
    [Serializable]
    public partial class ThreadSafeDictionary<[DynamicallyAccessedMembers(DynamicallyAccessedMembers.KeyWithHashingStrategy)]TKey, TValue>
        : IDictionaryInternal<TKey, TValue>, IDictionary, ISerializable, IDeserializationCallback
#if !(NET35 || NET40)
...
#pragma warning restore CS8618
        ...
        public bool ContainsValue(TValue value)
        {
            while (true)
            {
                ...
                bool found = lockFreeValues.ContainsValue(value);
        ...
        public bool TryRemove(TKey key)
        {
            ...
            while (true)
            {
                ...
                // found as already deleted: quick return with no change like in TryGetValueInternal
                if (success == false)
                    return false;
        ...
        private bool TryInsertInternal(TKey key, TValue value, uint hashCode, DictionaryInsertion behavior)
        {
            while (true)
            {
                ...
                // Duplicate key. No up-to-date check is needed here because in this case there was no change.
                if (success == false)
                    return false;

                lock (syncRoot)
                {
                    // lost race: must check the lock free values again to prevent possible duplicate keys
                    if (!IsUpToDateInLock(lockFreeValues))
                        continue;
        ...
        private bool TryReplaceInternal(TKey key, uint hashCode, TValue newValue, TValue originalValue)
        {
            while (true)
            {
                ...
                // Already deleted or originalValue does not match. Immediate return because in this case there was no change.
                if (success == false)
                    return false;

                // replaced in the fixed-size storage, or not found (success == null) and there is no locking storage
                if (success == true || expandableStorage == null)
                {
                    if (IsUpToDate(lockFreeValues))
                        return success.GetValueOrDefault();
